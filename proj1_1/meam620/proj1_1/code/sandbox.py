...
#import code.se3_control1 as se3_control1
import se3_control as se3_control
...
plt.subplot(2, 1, 1)
plt.plot(time, state['x'][:, 0], label='x')
#plt.plot(time, state['x'][:, 1], label='y')
#plt.plot(time, state['x'][:, 2], label='z')
plt.plot(time, flat['x'][:, 0], label='Desired x', alpha=0.5)
#plt.plot(time, flat['x'][:, 1], 'g--', label='Desired y', alpha=0.5)
#plt.plot(time, flat['x'][:, 2], 'b--', label='Desired z', alpha=0.5)
plt.xlabel('Time (s)')
...
roll = Rotation.from_quat(state['q']).as_euler('zxy', degrees=False)[:, 1] # Extract roll
roll_desired = Rotation.from_quat(control['cmd_q']).as_euler('zxy', degrees=False)[:, 1]
plt.subplot(2, 1, 2)
plt.plot(time, roll, label='Roll Angle')
plt.plot(time, roll_desired, label='Desired Roll Angle')
plt.xlabel('Time (s)')
plt.ylabel('Angle (rad)')
plt.title('Orientation (Roll) vs. Time')
plt.legend()
plt.grid(True)
