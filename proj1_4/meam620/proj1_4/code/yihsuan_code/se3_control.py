...
class SE3Control(object):
    ...
    def __init__(self, quad_params):
        ...
        self.kp_position = np.array([11, 11, 10]) # proportional gains for x, y, z control
        self.kd_position = np.array([5, 5, 5]) # derivation gains for x, y, z control

        # Define PD control gains for attitude (rotational dynamics)
        self.kp_phi = np.diag(np.array([400, 400, 20]))#2700 # Roll control P gain
        self.kd_phi = np.diag(np.array([20, 20, 10]))#350 # Roll control D gain
