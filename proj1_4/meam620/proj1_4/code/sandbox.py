...
from proj1_4.code.read_rosbag import output
...
desired_flight_distance = np.sum(np.linalg.norm(np.diff(flat['x'], axis=0),axis=1))
print("desired trajectory distance", desired_flight_distance)
...
#print("simulation flight distance", flight_distance)
print()
print(f"Results:")
...
world.draw_line(ax, actual_trajectory, color='blue', linewidth=2)
ax.legend(handles=[
    ...
    Line2D([], [], color='blue', linewidth=2, label='Actual Trajectory')],
    loc='upper right')
ax.set_xlabel('x (m)')
ax.set_ylabel('y (m)')
ax.set_zlabel('z (m)')

#print("my_world_traj.path", my_world_traj.path)
#print("flat['x']", flat['x'])
