...
while True:
    ...
    pose.append((nominal_state[2], nominal_state[0], nominal_state[1], nominal_state[3].copy(), nominal_state[4].copy(), nominal_state[5].copy()))
...
plt.title('Angular Velocity Bias')
plt.legend()
plt.show()
#%%
plt.figure()
plt.plot(g_[:, 0], label='gx')
plt.plot(g_[:, 1], label='gy')
plt.plot(g_[:, 2], label='gz')
plt.ylabel('radians per second ')
plt.title('Gravity')
plt.legend()
plt.show()